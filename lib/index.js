"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
// import { Proofreader } from 'proofreader'
var marked = require('marked');
var atob = require('atob');
var Proofreader = require('proofreader');
function nthIndex(str, pat, n) {
    var L = str.length;
    var i = -1;
    while (n-- && i++ < L) {
        i = str.indexOf(pat, i);
        if (i < 0)
            break;
    }
    return i;
}
function formatResults(results) {
    var prettyOutput = 'Hello, this is an Editor bot\n\n.';
    prettyOutput += 'The following is a list of grammar/style suggestions and';
    prettyOutput += ' possible spelling mistakes. These are just suggestions, but';
    prettyOutput += ' you should consider them all and push any changes to your';
    prettyOutput += ' pull request branch.\n\n';
    results.forEach(function (result) {
        var writeGood = result.suggestions.writeGood;
        var spelling = result.suggestions.spelling;
        if (writeGood.length || spelling.length) {
            prettyOutput += 'Your text: *' + result.text + '*\n';
            writeGood.forEach(function (item) {
                prettyOutput += ' - ' + item.reason + '\n';
            });
            spelling.forEach(function (item) {
                prettyOutput += ' - "*' + item.word + '*" -> ' + item.suggestions + '\n';
            });
            prettyOutput += '\n';
        }
    });
    return prettyOutput;
}
module.exports = function (app) {
    app.on(['pull_request.opened', 'pull_request.synchronized'], function (context) { return __awaiter(_this, void 0, void 0, function () {
        var owner, repo, number, prFiles, submitter, prRef, _i, _a, file, markdownFile, markdownEncoded, markdownWithHeader, endOfHeader, markdown, proofer;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    owner = context.payload.repository.owner.login;
                    repo = context.payload.repository.name;
                    number = context.payload.number;
                    return [4 /*yield*/, context.github.pullRequests.listFiles({
                            owner: owner,
                            repo: repo,
                            number: number,
                        })];
                case 1:
                    prFiles = _b.sent();
                    submitter = context.payload.pull_request.head.user.login;
                    prRef = context.payload.pull_request.head.ref;
                    _i = 0, _a = prFiles.data;
                    _b.label = 2;
                case 2:
                    if (!(_i < _a.length)) return [3 /*break*/, 5];
                    file = _a[_i];
                    markdownFile = file.filename;
                    if (!markdownFile.endsWith('.md')) {
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, context.github.repos.getContents({
                            owner: submitter,
                            repo: repo,
                            path: markdownFile,
                            ref: prRef
                        })
                        // Decode and skip the yaml header
                    ];
                case 3:
                    markdownEncoded = _b.sent();
                    markdownWithHeader = atob(markdownEncoded.data.content);
                    endOfHeader = nthIndex(markdownWithHeader, '---', 2) + 3;
                    markdown = markdownWithHeader.slice(endOfHeader);
                    proofer = new Proofreader();
                    proofer.proofread(marked(markdown))
                        .then(function (result) {
                        return __awaiter(this, void 0, void 0, function () {
                            var formattedResult, issueComment;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        formattedResult = formatResults(result);
                                        context.log(formattedResult);
                                        issueComment = context.issue({ body: formattedResult });
                                        return [4 /*yield*/, context.github.issues.createComment(issueComment)];
                                    case 1:
                                        _a.sent();
                                        return [2 /*return*/, result];
                                }
                            });
                        });
                    })
                        .catch(function (error) {
                        context.log('Proofreading failed', error);
                    });
                    _b.label = 4;
                case 4:
                    _i++;
                    return [3 /*break*/, 2];
                case 5: return [2 /*return*/];
            }
        });
    }); });
};
// https://github.com/koddsson/eslint-disable-probot/blob/master/index.js
//# sourceMappingURL=index.js.map